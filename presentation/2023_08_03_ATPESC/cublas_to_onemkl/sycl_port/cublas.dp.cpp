...
void initializeCUDA(int argc, char **argv, int &devID, int &iSizeMultiple,
    ...
    error = devID = dpct::dev_mgr::instance().current_device_id();
...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size)
{
    ...
    */
    int block_size = (deviceProp.get_major_version() < 2) ? 16 : 32;
    ...
    checkCudaErrors(
        DPCT_CHECK_ERROR(q_ct1.memcpy(d_A, h_A, mem_size_A).wait()));
    checkCudaErrors(
        DPCT_CHECK_ERROR(q_ct1.memcpy(d_B, h_B, mem_size_B).wait()));
    checkCudaErrors(DPCT_CHECK_ERROR(
    ...
    // CUBLAS version 2.0
    {
        ...
        checkCudaErrors(DPCT_CHECK_ERROR(handle = &q_ct1));
        ...
        /*
        DPCT1012:1: Detected kernel execution time measurement pattern and
        generated an initial code for time measurements in SYCL. You can change
        the way time is measured depending on your goals.
        */
        /*
        DPCT1024:2: The original code returned the error code that was further
        ...
        */
        ...
        for (int j = 0; j < nIter; j++)
        {
            ...
            checkCudaErrors(
                DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(
                    *handle, oneapi::mkl::transpose::nontrans,
                    oneapi::mkl::transpose::nontrans, matrix_size.uiWB,
        ...
        */
        /*
        DPCT1024:4: The original code returned the error code that was further
        consumed by the program logic. This original code was replaced with 0.
        You may need to rewrite the program logic consuming the error code.
        */
        ...
        checkCudaErrors(
            DPCT_CHECK_ERROR(q_ct1.memcpy(h_CUBLAS, d_C, mem_size_C).wait()));

        // Destroy the handle
        checkCudaErrors(DPCT_CHECK_ERROR(handle = nullptr));
    }
    ...
    free(h_C);
    free(reference);
    checkCudaErrors(DPCT_CHECK_ERROR(sycl::free(d_A, q_ct1)));
    checkCudaErrors(DPCT_CHECK_ERROR(sycl::free(d_B, q_ct1)));
    checkCudaErrors(DPCT_CHECK_ERROR(sycl::free(d_C, q_ct1)));
